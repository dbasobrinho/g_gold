...
create or replace package moats as
   ...
   -- Pipelined function to show TOP instance summary acTVTy. The refresh rate
...
--v_header := v_header||' |                                                                                                             +-+-+-+-+-+-+-+-+-+-+-+  | ';  
--v_header := v_header||' | ref    : https://github.com/dbsid/moats_rac                                                                 |d|b|a|s|o|b|r|i|n|h|o|  | ';  
--v_header := v_header||' |                                                                                                             +-+-+-+-+-+-+-+-+-+-+-+  | ';          
--v_header := v_header||' +--------------------------------------------------------------------------------------------------------------------------------------+ ';
--v_header := v_header||' ..	                                                                                                                                   ..';
      ...
      v_rows.extend(1); v_header :='----------------+ |                                                                                                      +-+-+-+-+-+-+-+-+-+-+-+         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ |                                                                                                      |d|b|a|s|o|b|r|i|n|h|o|         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');
      v_rows.extend(1); v_header :='----------------+ |                                                                                                      +-+-+-+-+-+-+-+-+-+-+-+         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');
      -->>
...
--      -- Set the instance summary output...
     ...
     --v_rows.extend(3);
	  ...
	  v_rows.extend(1);
      V_ROWS(V_ROWS.LAST):= MOATS_OUTPUT_OT(RPAD('+ DATABASE: ' ||  RPAD(V_DBNAME, 8)            || ' |' || RPAD(' ACTVTY STATISTICS PER SECOND',33) || '|' ||
