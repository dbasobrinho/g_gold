...
,    substr((select trim(replace(replace(substr(event,1,100),'SQL*Net'),'Streams')) from gv$session_wait j where j.sid = s.sid and j.INST_ID =  s.inst_id),1,24) as sessionwait, s.username
from gv$session s
,        gv$process p
,    gv$px_session e
...
where z.username = '&ENTER_USERNAME'
/

COLUMN username           FORMAT A26    
alter session set container = PDBSISCOMP;
--ERSECOMGCP
